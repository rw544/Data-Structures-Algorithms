...
public class Heap<E> {
    ...
    public static double timeSpent= 7.0;
    ...
    /** If size = length of array b, double the length of array b. <br>
    ...
    protected void ensureCapacity() {
        ...
        if (size >= b.length) {
            Element[] d;
            d= createElementArray(b.length * 2);
            int index= 0;
            for (Element elt : b) {
                d[index++ ]= elt;
            }
    ...
    void bubbleDown(int h) {
        ...
        while (child < size) {
            if (rChild < size && compareTo(rChild, child) == 1) {
                child= rChild;
            }
            if (compareTo(child, h) == 1)
                swap(h, child);
            else
                break;
    ...
    public E poll() {
        ...
        // test33Poll, test34Poll, test40testDuplicatePriorities
        if (map.isEmpty()) { throw new NoSuchElementException(); }
        E elt= peek();
    ...
    public void updatePriority(E v, double p) {
        ...
        if (map == null || !map.containsKey(v)) {
            ...
            double prevPriority= b[map.get(v)].priority;
            b[map.get(v)].priority= p;
            if (prevPriority == p) {
                return;
            } else if (compareTo(p, prevPriority) == 1) {
                bubbleUp(map.get(v));
            } else {
